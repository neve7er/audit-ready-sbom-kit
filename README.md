...
🧠 Core guarantees
🧩 Pure functions only
No Date, no Math.random(), no environment variables
🔁 Same input = same output
Reproducible across machines and CI
📜 Rule order defines outcome
...
Every component receives exactly one reasonCode, such as:
...
Human-readable risk levels (Critical, NeedsReview, Acceptable) are:

independent from reasonCode
not used for policy decisions
included only for reporting clarity
🔍 Why this matters
